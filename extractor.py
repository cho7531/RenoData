...
이 문서는 특정 시점에 발급된 등기부등본입니다. 반드시 "발급 시점 기준으로 유효한 현재 상태"만 추출하세요.
...
현재 유효한 소유자가 여러 명(공유)인 경우 owners 배열에 각각 별도 항목으로 모두 포함하세요.
소유자 정보를 전혀 찾을 수 없으면 owners를 빈 배열로 두세요."""
        ...
        rows.append(
            {
                ...
                "해당필지 지번": data.get("해당필지_지번", ""),
                "호실": data.get("호실", ""),
                "대지면적": data.get("대지면적", ""),
                "건축연면적": data.get("건축연면적", ""),
                "파일명": filename,
            }
        )
    return rows
